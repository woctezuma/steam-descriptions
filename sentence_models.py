...
def filter_out_words_not_in_vocabulary(
    tokenized_sentence,
    index2word_set=None,
    wv_spacy=None,
):
    if index2word_set is not None:
        ...
        filtered_tokenized_sentence = [
            word for word in tokenized_sentence if word in index2word_set
        ]
    else:
        # Using spaCy for the GloVe word embedding
        filtered_tokenized_sentence = [
            word for word in tokenized_sentence if wv_spacy[word].has_vector
        ]
...
def get_store_url_as_bb_code(app_id):
    store_url = (
        '[URL=https://store.steampowered.com/app/' + app_id + ' ]' + app_id + '[/URL]'
    )
    return store_url


def print_most_similar_sentences(
    similarity_scores,
