...
def compute_retrieval_score_based_on_sharing_meta_data(
    query_app_ids,
    ...
    for query_counter, query_app_id in enumerate(query_app_ids):
        ...
        for rank, app_id in enumerate(reference_app_id_counter):
            ...
            if rank >= (num_elements_displayed - 1):
                ...
                if verbose:
                    print(
                        '[appID={}] retrieval score based on sharing meta-data = {}'.format(
                            query_app_id,
    ...
    print(
        'Total retrieval score based on sharing {} = {}'.format(
            meta_data_str,
            retrieval_score,
        ),
    )
...
def compute_retrieval_score_based_on_sharing_genres(
    query_app_ids,
    reference_app_id_counters,
    num_elements_displayed=10,
    verbose=True,
    ...
    retrieval_score = compute_retrieval_score_based_on_sharing_meta_data(
        query_app_ids,
        reference_app_id_counters,
        retrieval_ground_truth,
...
def compute_retrieval_score_based_on_sharing_tags(
    query_app_ids,
    reference_app_id_counters,
    num_elements_displayed=10,
    verbose=True,
